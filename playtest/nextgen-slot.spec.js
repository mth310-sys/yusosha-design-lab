...
test('PHASER BASELINE v0.4 full 1G iPhone interaction', async ({page})=>{
 ...
 const initialReel=await reelState(page);
 expect(initialReel?.version).toBe('0.2.3');
 expect(initialReel?.lineCount).toBe(3);
 expect(initialReel?.lineYs).toEqual([306,342,378]);
 ...
 for(let i=0;i<3;i++){
   ...
   await waitEvent(page,`REEL_ALIGN  R${i+1}`);
   const rs=await reelState(page); const locked=rs.reels[i];
   expect(locked.mode).toBe('locked');
   expect(locked.latticeError).toBeLessThanOrEqual(.001);
   expect(locked.lineErrors.every(v=>v!==null && v<=.001)).toBe(true);
   reelChecks.push({stop:i+1,state:rs});
 ...
 expect(report.reel?.lineCount).toBe(3); expect(report.reel?.reels.every(r=>r.latticeError<=.001 && r.lineErrors.every(v=>v!==null && v<=.001))).toBe(true);
